        ...
        if st.session_state.get('preprocessing_done', False):
            ...
            if stats:
                ...
                <div class="metric-grid-4">
                    <div class="metric-card-pro"><div class="metric-num txt-indigo">{stats['raw']:,}</div><div class="metric-txt">Data Mentah</div></div>
                    <div class="metric-card-pro"><div class="metric-num txt-emerald">{stats['awal']:,}</div><div class="metric-txt">Filter Duplikat</div></div>
                    <div class="metric-card-pro"><div class="metric-num txt-amber">{stats['segmentasi']:,}</div><div class="metric-txt">Hasil Segmentasi</div></div>
                    <div class="metric-card-pro"><div class="metric-num txt-emerald">{stats['final']:,}</div><div class="metric-txt">Segmen Bersih Final</div></div>
                </div>
                """, unsafe_allow_html=True)

            st.divider()
            st.markdown("### Preview Hasil Data Akhir (Doc ID & Segmen Clean)")
...
# ============================================================
    ...
    if st.session_state['df_exploded'] is not None:
        ...
        st.markdown("### Pengaturan Penanganan Sentimen Netral")
        ...
        if not st.session_state['labeling_done']:
            ...
            with c1:
                st.markdown("##### Distribusi Sentimen Biner")
            ...
            with c2:
                st.markdown("##### Distribusi Aspek Opini")
